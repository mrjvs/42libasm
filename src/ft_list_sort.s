...
_ft_list_sort:
	...
	je .return
	mov r10, QWORD rdi			; save parent pointer to reg 10
	mov r8, QWORD rdi			; save beginning of the list
	mov rdi, QWORD [rdi]		; derefrence list once
	cmp rdi, 0					; check if list item is NULL pointer, return if it is
	je .return
	mov r12, QWORD [rdi+8]		; save next list item
...
.sizeloop:
	...
	mov rax, QWORD [rax+8]		; set rdi to next list item
...
.skipswap:
	...
	mov r12, QWORD [rdi+8]		; set reg 12 to next next list item
	jmp .listloop				; jump to start of loop

.endlistloop:
	mov rdi, QWORD [r8]			; reset rdi to beginning of list
	...
	jg .listloop				; do list loop if counter did is greater than one
